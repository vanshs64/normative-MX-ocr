...
def openai_extract(image_paths, key_template):
    ...
    for image_path in tqdm(image_paths):
        ...
        response = client.responses.create(
            ...
            input=[
                {"role": "user", "content": [
                    {f"type": "input_text", "text": "Extract all text from this image. We are looking for a key-value mapping to pair text data in the image to this following structure: {reference_text}. Once each of these in a similar dictionary format."},
...
def main():
    ...
    for name in tqdm(names):
        ...
        for doc_type in tqdm(doc_types):
            ...
            key_template = ""
            with open(key_template_path, "r") as template_file:
                key_template = template_file.read()
            ...
            save_text_to_file(extracted_text, f"../test_docs/test_study_data/{name}Hyp", f"{doc_type}{name}Hyp.txt")
